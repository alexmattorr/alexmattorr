<section class="contact" id="contact">
	<!-- <h6>I know your not going to fill out a form:</h6> -->
	<div class="contact-image" style="background-image: url('<?php echo get_template_directory_uri(); ?>/dist/assets/img/hs3.jpg');"></div>
	<div class="contact-info">
		<h3 class="title">contact</h3>
		<div class="contact-item">
			<a href="mailto:alexmattorr@gmail.com">alexmattorr@gmail.com</a>
		</div>

		<div class="contact-item">
			<a href="tel:4153149497">(415) 314 9497</a>
		</div>

		<div class="contact-item contact-location">
			San Francisco, <span>CA</span>
		</div>

		<ul class="contact-social">
			<li>
				<a href="https://github.com/alexsmander" target="_blank">
					<i class="fa fa-github-alt fa-lg"></i>
				</a>
			</li>

			<li>
				<a href="https://www.linkedin.com/in/alexmattorr" target="_blank">
					<i class="fa fa-linkedin fa-lg"></i>
				</a>
			</li>

			<li>
				<a href="https://twitter.com/alexmattorr" target="_blank">
					<i class="fa fa-twitter fa-lg"></i>
				</a>
			</li>
		</ul>

	</div>
</section>